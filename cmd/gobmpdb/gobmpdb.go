...
import (
	...
	"github.com/sbezverk/gobmp/pkg/bmppg"
...
func init() {
	...
	flag.IntVar(&timeout, "timeout", 300000, "timeout in seconds, default 300, for the test to complete all processing.")
	...
	flag.StringVar(&pgbmphost, "pgbmphost", "127.0.0.1", "")
	flag.StringVar(&pgbmpdb, "pgbmpdb", "bmp", "")

}

func main() {
	bmppg.Pg_host = pgbmphost
	bmppg.Pg_db = pgbmpdb
	bmppg.Open()
	flag.Parse()
	...
	*/
	...
	for _, test := range tests {
		...
		case "u4":
			/*
				...
				topics = append(topics, &kafka.TopicDescriptor{
					TopicName: kafka.L3vpnMessageV4Topic,
			...
			*/
			for _, kt := range kts {
				topics = append(topics, &kafka.TopicDescriptor{
					TopicName: kt.TopicName,
					TopicType: kt.TopicTypeId,
					TopicChan: make(chan []byte),
				})
			}
		default:
	...
	if validatorFlag {
		go bmppg.Check(topics, b, stopCh, errCh)
	} else {
		//go bmppg.Store(topics, f, stopCh, errCh)
		go bmppg.Store(topics, stopCh, errCh)
	}
